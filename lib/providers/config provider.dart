  ...
  class ConfigProvider extends ChangeNotifier {
          ...
          if (user.authSuccess) {
            ...
            if (settingsResponse.statusCode == 200) {
              ...
              updatePreferences(
                      email,
                      settings.name ?? "",
                      settings.dateOfBirth ?? "00-00-00",
                      settings.gender ?? "MALE",
                      settings.iProfileShared)
                  .then((value) {
                myemail = email;
    ...
    Future<bool> updateProfileSettings(
        ...
        String gender,
        String birthDate,
        bool isProfileShared) async {
      print(gender);
      try {
        var response = await WebServices.updateSettings(
            sessionKey, name, gender, birthDate, isProfileShared);

        print(response.body);
        if (response.statusCode == 200 && jsonDecode(response.body)["success"]) {
          updatePreferences(email, name, birthDate, gender, isProfileShared)
              .then((value) {
            myemail = email;
            myname = name;
 ...
 void setUserInfo() {
    ...
    Future<void> updatePreferences(String email, String name, String birthDate,
        String gender, bool isShared) async {
